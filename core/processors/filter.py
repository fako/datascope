...
class FilterProcessor(QuerySetProcessor):
    ...
    def get_query_filter_for_documents(self, criteria):
        query_filter = Q()
    ...
    def get_query_filter_for_individuals(self, criteria):
        query_filter = Q()
        for key, values in criteria.items():
            for value in values:
    ...
    def filter(self, query_set):
        ...
        query_filter = self.get_query_filter_for_documents(criteria) if issubclass(query_set.model, DocumentPostgres) \
            else self.get_query_filter_for_individuals(criteria)
