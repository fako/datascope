...
def deserialize(serialization):
    try:
        ...
        assert isinstance(serialization[0], (unicode, str)), \
            "HIFImproperUsage: model in serialization tuple is not stringish but {}.".format(type(serialization[0]))
        assert isinstance(serialization[1],(int, long, float, complex)), \
            "HIFImproperUsage: object id in serialization is not an numberish, but {}.".format(type(serialization[1]))
    except IndexError:
